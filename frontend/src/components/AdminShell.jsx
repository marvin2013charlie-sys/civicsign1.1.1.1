...
const NAV = [
  { to: "/admin", end: true, label: "Overview", icon: LayoutDashboard, testid: "admin-nav-overview" },
  { to: "/admin/users", label: "Users", icon: Users, testid: "admin-nav-users" },
  { to: "/admin/envelopes", label: "Envelopes", icon: FileText, testid: "admin-nav-envelopes" },
  { to: "/admin/blog", label: "Blog", icon: BookOpen, testid: "admin-nav-blog" },
  { to: "/admin/team", label: "Internal Team", icon: UserCog, testid: "admin-nav-team" },
  { to: "/admin/billing", label: "Billing & Refunds", icon: CreditCard, testid: "admin-nav-billing" },
  { to: "/admin/audit", label: "Audit Log", icon: History, testid: "admin-nav-audit" },
  { to: "/admin/contacts", label: "Contact Inbox", icon: Inbox, testid: "admin-nav-contacts" },
];
    ...
    <div className="flex h-full flex-col text-white">
      <div className="flex items-center gap-2 px-5 py-5">
        ...
        <div className="leading-tight">
          ...
          <p className="text-[10px] font-semibold uppercase tracking-widest text-white/50">Admin Console</p>
        </div>
      </div>
      ...
      <div className="lg:pl-64">
        <header className="sticky top-0 z-30 flex h-16 items-center gap-3 border-b border-[var(--c-border)] bg-[var(--c-paper)]/90 px-4 backdrop-blur sm:px-6">
          ...
          <span className="inline-flex items-center gap-1.5 rounded-full border border-[var(--c-border)] bg-[var(--card)] px-3 py-1 text-xs font-semibold text-[var(--c-ink)]">
            <ShieldCheck className="h-3.5 w-3.5" style={{ color: "var(--c-primary)" }} /> Internal team only
          </span>
        </header>
