...
export default function AdminUserDetail() {
  ...
  const patch = async (body) => {
    setSaving(true);
    try {
      const { data: fresh } = await api.patch(`/admin/users/${userId}`, body);
      setData((d) => ({ ...d, user: { ...d.user, ...fresh } }));
  ...
  const { user, stats, recent_envelopes: recent, diagnostics } = data;
  ...
  return (
    <div data-testid="admin-user-detail" className="space-y-6">
      <button
        ...
        className="inline-flex items-center gap-1.5 text-sm font-medium text-[var(--muted-foreground)] transition-colors hover:text-[var(--c-ink)]"
      >
      ...
      <div className="flex flex-wrap items-center gap-4 rounded-xl border border-[var(--c-border)] bg-[var(--card)] p-5">
        ...
        <div className="min-w-0 flex-1">
          ...
          <p className="truncate text-sm text-[var(--muted-foreground)]" data-testid="admin-user-detail-email">{user.email}</p>
      ...
      <div className="grid gap-6 lg:grid-cols-2">
          ...
          <div className="mt-4 space-y-4">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-xs text-[var(--muted-foreground)]">Subscription tier</p>
              </div>
            ...
            <div className="flex items-center justify-between border-t border-[var(--c-border)] pt-4">
              <div>
                ...
                <p className="text-xs text-[var(--muted-foreground)]">Disabled users can't sign in</p>
              </div>
              <Switch
            ...
            <div className="flex items-center justify-between border-t border-[var(--c-border)] pt-4">
              <div>
                ...
                <p className="text-xs text-[var(--muted-foreground)]">Admin access is provisioned on the backend only</p>
              </div>
        ...
        <div className="rounded-xl border border-[var(--c-border)] bg-[var(--card)] p-5" data-testid="admin-detail-diagnostics">
          ...
          <p className="mt-0.5 text-sm text-[var(--muted-foreground)]">Account health & delivery status.</p>
          ...
          <div className="mt-4 grid grid-cols-4 gap-3">
            {STAT_TILES.map((t) => (
              <div key={t.key} className="rounded-lg border border-[var(--c-border)] bg-[var(--c-paper-2)] p-3 text-center">
                <t.icon className="mx-auto h-4 w-4 text-[var(--muted-foreground)]" />
                <p className="mt-1 font-heading text-xl font-bold text-[var(--c-ink)]">{stats[t.key]}</p>
                <p className="text-[11px] text-[var(--muted-foreground)]">{t.label}</p>
              </div>
            ))}
          </div>
          <div className="mt-4">
            <p className="mb-2 text-xs font-semibold uppercase tracking-wide text-[var(--muted-foreground)]">Recent envelopes</p>
            {recent.length === 0 ? (
              <p className="rounded-lg bg-[var(--c-paper-2)] px-3 py-4 text-center text-sm text-[var(--muted-foreground)]">No envelopes yet.</p>
            ) : (
              <ul className="space-y-1.5" data-testid="admin-detail-recent">
                {recent.map((e) => (
                  <li key={e.envelope_id} className="flex items-center justify-between rounded-lg border border-[var(--c-border)] px-3 py-2">
                    <span className="truncate text-sm text-[var(--c-ink)]">{e.title}</span>
                    <span className="ml-3 shrink-0 rounded-full px-2 py-0.5 text-[11px] font-medium capitalize" style={{ background: "var(--c-paper-2)", color: "var(--muted-foreground)" }}>{e.status}</span>
                  </li>
                ))}
              </ul>
            )}
        ...
        <div className="rounded-xl border border-[var(--c-border)] bg-[var(--card)] p-5" data-testid="admin-detail-support">
          ...
          <div className="mt-4 rounded-lg border border-[var(--c-border)] p-4">
            ...
            {isGoogle && (
              <p className="mt-2 text-xs text-[var(--muted-foreground)]">This account signs in with Google — there's no password to reset.</p>
            )}
            {resetLink && (
